...
/**
 * Get the number of CSV blocks from a CSV 2of2 multisig scriptPubkey.
 *
 * Returns the number of blocks that must have passed since confirmation
 * of the transaction input before the recovery key alone can be used
 * to spend the input. Can be used with scriptPubkeys generated by
 * either `wally_scriptpubkey_csv_2of2_then_1_from_bytes`
 * or `wally_scriptpubkey_csv_2of2_then_1_from_bytes_opt`.
 *
 * :param bytes: The scriptPubkey to fetch the CSV blocks from.
 * :param bytes_len: Length of ``bytes`` in bytes.
 * :param value_out: Destination for the resulting number of CSV
 *|    blocks (between 17 and 65536).
 */
WALLY_CORE_API int wally_scriptpubkey_csv_blocks_from_csv_2of2_then_1(
    const unsigned char *bytes,
    size_t bytes_len,
    uint32_t *value_out);
